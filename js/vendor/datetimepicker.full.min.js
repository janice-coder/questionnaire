  ...
  $.fn.datetimepicker = function (opt) {
    ...
    var createDateTimePicker = function (input) {
      var datetimepicker = $('<div class="xdsoft_datetimepicker"></div>'),
        xdsoft_copyright = $(
          '<div class="xdsoft_copyright"><a target="_blank" href="https://xdsoft.net/jqplugins/datetimepicker/">xdsoft.net</a></div>'
        ),
              ...
              input.on('keydown.xdsoft', function (event) {
                ...
                switch (true) {
                  case (key >= 48 && key <= 57) ||
                    ...
                    break;
                  case [AKEY, CKEY, VKEY, ZKEY, YKEY].indexOf(key) !== -1 &&
                    ctrlDown:
                  case [
                    ESC,
                    ARROWUP,
      ...
      datetimepicker.on('change.xdsoft', function () {
        ...
        for (var j = 0; j < 7; j++)
          table +=
            ...
            options.i18n[options.lang].dayOfWeek[
              j + options.dayOfWeekStart > 6 ? 0 : j + options.dayOfWeekStart
            ] +
            '</th>';
        table += '</tr></thead>';
        table += '<tbody><tr>';
        while (
          ...
          table +=
            ...
            (options.minDate !== false &&
              Math.round(xd.strtodate(options.minDate).getTime() / 86400000) >
                Math.round(start.getTime() / 86400000))
              ? 'xdsoft_disabled '
              : ' ') +
            (xd.currentTime.getMonth() != start.getMonth()
              ? ' xdsoft_other_month '
              : ' ') +
            (xd.currentTime.dateFormat('d.m.Y') == start.dateFormat('d.m.Y')
              ? ' xdsoft_current '
              : ' ') +
        ...
        mounth_picker
          .find('.xdsoft_label')
          .eq(0)
          .text(options.i18n[options.lang].months[xd.currentTime.getMonth()]);
        mounth_picker
          .find('.xdsoft_label')
          .eq(1)
        ...
        var line_time = function (h, m) {
          ...
          time +=
            ...
            ((options.maxTime !== false &&
              xd.strtotime(options.maxTime).getTime() < now.getTime()) ||
            (options.minTime !== false &&
              xd.strtotime(options.minTime).getTime() > now.getTime())
              ? 'xdsoft_disabled '
              : ' ') +
        ...
        if (onShow !== false) {
          var setPos = function () {
            var offset = datetimepicker.data('input').offset(),
              top =
                offset.top + datetimepicker.data('input')[0].offsetHeight - 1;
        ...
        .on('open.xdsoft focusin.xdsoft mousedown.xdsoft', function (event) {
          ...
          timer = setTimeout(function () {
            if (
              ...
              return;
            _xdsoft_datetime.setCurrentTime(
              input && input.val && input.val() ? input.val() : new Date()
            );
